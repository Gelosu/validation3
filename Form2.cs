...
using static System.Windows.Forms.DataFormats;
    ...
    public partial class Form2 : Form
    {
        ...
        {
            string logFilePath = "db_insert_log.txt";
            int totalRowsProcessed = 0;
            int totalRowsInserted = 0;
            int totalRowsUpdated = 0;
            int totalMasterlistInserted = 0;
                            ...
                            using (MySqlCommand checkMasterlistCmd = new MySqlCommand(checkMasterlistQuery, connection))
                            {
                                ...
                                if (masterlistCount == 0)
                                {
                                    ...
                                    using (MySqlCommand insertMasterlistCmd = new MySqlCommand(insertMasterlistQuery, connection))
                                    {
                                        ...
                                        totalMasterlistInserted++;
                                        logWriter.WriteLine($"Inserted into masterlist: {qrcode}, {tupcid}, {uid}");
                                    }
                                }
                            }
                            ...
                            using (MySqlCommand checkCmd = new MySqlCommand(checkQuery, connection))
                            {
                                ...
                                if (count > 0)
                                {
                                    ...
                                    using (MySqlCommand updateCmd = new MySqlCommand(updateQuery, connection))
                                    {
                                        ...
                                        totalRowsUpdated++;
                                        logWriter.WriteLine($"Updated {tableName}: {qrcode}, {tupcid}, {uid}");
                                    }
                                }
                                else
                                {
                                    ...
                                    using (MySqlCommand insertCmd = new MySqlCommand(insertQuery, connection))
                                    {
                                        ...
                                        totalRowsInserted++;
                                        logWriter.WriteLine($"Inserted into {tableName}: {qrcode}, {tupcid}, {uid}");
                                    }
                                }
                            }
                        }

                        logWriter.WriteLine($"Total rows processed: {totalRowsProcessed}");
                        logWriter.WriteLine($"Total rows inserted into {tableName}: {totalRowsInserted}");
                        logWriter.WriteLine($"Total rows updated in {tableName}: {totalRowsUpdated}");
                        logWriter.WriteLine($"Total rows inserted into masterlist: {totalMasterlistInserted}");
                    }
                    catch (Exception ex)
                    {
                        logWriter.WriteLine($"Failed to insert/update data. Error: {ex.Message}");
